function [onts] = pfp_ontbuild(obofile, rel)
    ...
    % Note
    % ----
    % This "parser" is NOT meant to fully support OBO format 1.2. It considers
    % the OBO file format to be the following, which is suffice to build a DAG.
    %
    ...
    % 6. Cross-ontology/namespace relationships are ignored (for example in GO)
    %
    % Input
    % -----
    % (required)
    ...
    %           relationship codes in the given OBO file are ignored. It is
    %           passed to the output as one of its field: 'rel_code'.
    ...
    %           If the given OBO file only has one ontology structure, 'onts' is
    %           a single variable of type 'struct'; while multiple ontologies
    %           result in a cell array of ontology structures. In either cases,
...
% Last modified: Wed 21 Sep 2016 01:15:15 PM E
