function [cm] = pfp_seqcm(target, pred, oa, varargin)
    ...
    %
    %   Calculate sequence-centric confusion matrices. It computes k matrices
    %   for each 'target' sequence, where 'k' is the number of thresholds.
    %
    % Note
    % ----
    % This function assumes the predicted scores have been normalized to be
    ...
    %           predicted associtated scores for each term will be 0.00, as if
    ...
    %
    % [logical or char]
    % 'toi' A binary vector indicating "terms of interest". Note that the
    %       following special short-hand tokens are also allowed.
    %       'all'    - all terms
    ...
    %       .npp      [double]  An n-by-1 array of "# of positive predictions"
    %                           for each sequence.
...
% Last modified: Wed 21 Sep 2016 01:46:46 PM E
